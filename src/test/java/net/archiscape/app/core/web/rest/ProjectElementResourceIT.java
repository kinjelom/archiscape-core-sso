...
class ProjectElementResourceIT {
    ...
    void createProjectElement() throws Exception {
        ...
        ProjectElementDTO projectElementDTO = projectElementMapper.toDto(projectElement);
        restProjectElementMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(projectElementDTO))
            )
    ...
    void putNewProjectElement() throws Exception {
        ...
        updatedProjectElement
            ...
            .documentation(UPDATED_DOCUMENTATION)
            .landscapeElementId(UPDATED_LANDSCAPE_ELEMENT_ID);
        ProjectElementDTO projectElementDTO = projectElementMapper.toDto(updatedProjectElement);

        restProjectElementMockMvc
            .perform(
                put(ENTITY_API_URL_ID, projectElementDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(projectElementDTO))
            )
    ...
    void patchNonExistingProjectElement() throws Exception {
        ...
        restProjectElementMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, projectElementDTO.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(projectElementDTO))
            )
